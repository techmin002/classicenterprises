...
class AmcAssignController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // dd($branch->branch_code);

        if ($request->customer_id) {
            $customer = Customer::with('lead')->findOrFail($request->customer_id);
            $userName = $customer->user_name;
        } else {
            $cleanName = strtoupper(str_replace(' ', '', $request->customer_name));
            $userName = $branch->branch_code . $cleanName;
        }
        // dd($userName);
        // 🧾 Handle Receipts
    ...
    {
        $customer = AmcCustomer::with(['customer', 'amc'])->findOrFail($id);
        $customer->created_time = $this->formatTimeDifference($lead->created_at);
