...
@section('content')
        ...
        <section class="content">
            <div class="container-fluid">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <table id="example3" class="table table-bordered table-striped datatable-custom">
                                    ...
                                    <tbody>
                                        @forelse ($amccustomer as $key => $item)

                @php
                    // AMC period total months
                    $totalMonths = ($item->amc->year * 12) + $item->amc->month;
                    ...
                    // Last service date
                    $lastDate = \Carbon\Carbon::parse($item->last_date);

                    // Next service date (4 months after last service)
                    $nextServiceDate = $lastDate->copy()->addMonths(4);
                @endphp
                ...
                <tr>
                    ...
                    <td>{{ $item->contact }}</td>
                    <td>{{ $item->amc->title }}</td>
                    <td>{{ $lastDate->format('d-m-Y') }}</td>
                    <td class="text-danger fw-bold">{{ $nextServiceDate->format('d-m-Y') }}</td>
                    <td>{{ $amcEndDate->format('d-m-Y') }}</td>

                    <td>
                        @if(now()->greaterThanOrEqualTo($nextServiceDate->copy()->subDays(2)))
                            <span class="badge bg-danger">Service Due</span>
                        @else
                            <span class="badge bg-success">Not Due</span>
                        @endif
                    </td>
                </tr>
    ...
    <td>
        <a href="" class="btn btn-primary btn-sm" data-toggle="modal"
           data-target="#exampleModalamc{{ $customer['row_id'] }}">
           Create Ticket
        </a>

        {{-- MODAL --}}
        <div class="modal fade" id="exampleModalamc{{ $customer['row_id'] }}" tabindex="-1">
                ...
                <div class="modal-content">
                    ...
                    <form action="{{ route('amccustomer-ticket.store') }}" method="POST">
                        ...
                        <div class="modal-body">
                            ...
                            <input type="hidden" name="customer_id" value="{{ $customer['row_id'] }}">
                            ...
                            @php
                                $warrantyExpiry = Carbon\Carbon::parse($customer['assign_date'])->addYear();
