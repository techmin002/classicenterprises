...
<tr>
    ...
    <td>{{ \Carbon\Carbon::parse($customer->last_date)->addMonths(4)->format('d-m-Y') }}</td>
    <td>
        @php
            $nextServiceDate = \Carbon\Carbon::parse($customer->last_date)->addMonths(4);
            $showFrom = $nextServiceDate->copy()->subDays(2);
        @endphp
        @if(now()->toDateString() >= $showFrom->toDateString())
