...
class AmcCustomerController extends Controller
{
    ...
    public function service()
    {
        ...
        // All assigned AMC customers
        $amcCustomers = AmcCustomer::with(['customer', 'amc'])
            ->where('branch_id', $branch_id)
            ->whereIn('status', ['create', 'report', 'complete'])
            ->get();
        ...
        foreach ($amcCustomers as $item) {
            ...
            // 1) AMC ka total period
            $totalMonths = ($item->amc->year * 12) + ($item->amc->month);

            // 2) AMC start & end date
            $startDate = Carbon::parse($item->date);
            $amcEndDate = $startDate->copy()->addMonths($totalMonths);

            // 3) Last service
            $lastServiceDate = Carbon::parse($item->last_date);

            // 4) Next service date (4 months later)
            $nextServiceDate = $lastServiceDate->copy()->addMonths(4);

            // 5) Show record 2 days before next service date
            $showFromDate = $nextServiceDate->copy()->subDays(2);

            // 6) Conditions
            if (
                now()->greaterThanOrEqualTo($showFromDate) &&  // aaj date service ko show karne layak ho
                $nextServiceDate->lessThanOrEqualTo($amcEndDate) // AMC timeperiod ke andar hona chahiye
            ) {
                $finalData[] = $item;
            }
        }
