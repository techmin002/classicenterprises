    ...
    public function service()
{
    ...
    // Fetch all AMC customers with customer & AMC relation
    ...
    foreach ($amcCustomers as $item) {
        ...
        // Next service date (last_date + 4 months)
        $nextServiceDate = $lastDate->copy()->addMonths(4);

        // Show from date (2 days before next service)
        $showFrom = $nextServiceDate->copy()->subDays(2);

        // ✅ Filter: Only customers whose service date is due
        if ($nextServiceDate->toDateString() <= $amcEndDate->toDateString()
            && now()->toDateString() >= $showFrom->toDateString()) {
            $finalData[] = $item;
        }
    }

    // Pass only service-due customers to view
