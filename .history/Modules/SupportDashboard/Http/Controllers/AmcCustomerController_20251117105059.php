    ...
    public function service()
    {
        ...
        foreach ($amcCustomers as $item) {
            ...
            dd([
                ...
                'next_service' => $nextServiceDate->format('Y-m-d'),
                'show_from' => $showFrom->format('Y-m-d'),
                'today' => now()->format('Y-m-d'),
                'amc_end' => $item->amc ? \Carbon\Carbon::parse($item->date)->addMonths(($item->amc->year * 12 + $item->amc->month))->format('Y-m-d') : null,
            ]);
        }
        $today = Carbon::now();
