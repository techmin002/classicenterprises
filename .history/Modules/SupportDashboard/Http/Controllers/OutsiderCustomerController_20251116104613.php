...
class OutsiderCustomerController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $totalcustomer = CustomerTicket::where('branch_id', $branchId)->where('type', 'outs')->count();
        $queuecount = CustomerTicket::where('branch_id', $branchId)->where('type', 'outs')->where('status', 'queue')->count();
        $assigncount = CustomerTicket::where('branch_id', $branchId)->where('type', 'outs')->where('status', 'assign')->count();
        $completecount = CustomerTicket::where('branch_id', $branchId)->where('type', 'outs')->where('status', 'complete')->where('due_amount', 0)->count();

        return view('supportdashboard::outs_customer.dashboard', compact('totalcustomer', 'queuecount', 'assigncount', 'completecount'));
    }
    ...
    public function customercreate(Request $request)
    {
        ...
        return redirect()->back()->with('success', 'Outsider Customer Created Successfully.');
    }
    ...
    public function queue()
    {
        ...
        $customers = CustomerTicket::with(['amc', 'customer'])->where('branch_id', $branch_id)->where('type', 'outsider')->where('status', 'queue')->latest()->get();
    ...
    public function report()
    {
        ...
        $users = User::where('branch_id', $branch_id)->get();
        $customers = CustomerTicket::with(['amc', 'customer', 'user'])->where('branch_id', $branch_id)->where('type', 'outsider')->where('status', 'complete')->latest()->get();
