    ...
    public function service()
{
    ...
    foreach ($amcCustomers as $item) {
        ...
        // Next service date = last date + 4 months
        $nextServiceDate = $lastDate->copy()->addMonths(4);

        // Show from = 2 days before next service
        $showFrom = $nextServiceDate->copy()->subDays(2);

        // Show only if next service is within AMC period AND today >= showFrom
        if ($nextServiceDate->lessThanOrEqualTo($amcEndDate) && now()->greaterThanOrEqualTo($showFrom)) {
            $finalData[] = $item;
        }
    }
