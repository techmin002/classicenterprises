    ...
    public function service()
{
    ...
    // Load AMC customers with AMC details
    ...
    foreach ($amcCustomers as $item) {
        ...
        // AMC total period
        $totalMonths = ($amc->year * 12) + $amc->month;

        // AMC Assign last date
        $lastDate = \Carbon\Carbon::parse($item->last_date);

        // Generate every 4-month services
        $serviceDates = [];
        $addMonths = 4;

        while ($addMonths <= $totalMonths) {

            $serviceDates[] = $lastDate->copy()->addMonths($addMonths);
        ...
        $serviceData[] = [
            ...
            'customer_name' => $item->type == 'register'
                ? optional($item->customer->lead)->name
                : $item->customer_name,

            'contact'       => $item->type == 'register'
                ? optional($item->customer->lead)->mobile
                : $item->contact,

            'amc_name'      => $amc->title,
            'amc_price'     => $amc->price,
            'paid_amount'   => $item->amount,
            'remaining'     => $amc->price - $item->amount,
            'amc_period'    => "{$amc->year} Years {$amc->month} Months",
            'assign_date'   => $lastDate,
    ...
    return view('supportdashboard::amc_customer.service', compact('serviceData'));
}
