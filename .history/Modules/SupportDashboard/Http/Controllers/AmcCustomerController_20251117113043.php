...
class AmcCustomerController extends Controller
{
    ...
    // last_date + 4 months = next service date
    $nextServiceDate = \Carbon\Carbon::parse($item->last_date)->addMonths(4);

    // showFrom = next service - 2 days
    ...
    dump([
        ...
        'nextServiceDate' => $nextServiceDate->toDateString(),
        'showFrom'        => $showFrom->toDateString(),
        'today'           => now()->toDateString(),
        'amcEndDate'      => $amcEndDate->toDateString(),
        'cond1_next<=amc' => $nextServiceDate->toDateString() <= $amcEndDate->toDateString(),
        'cond2_today>=showFrom' => now()->toDateString() >= $showFrom->toDateString(),
    ]);
}
dd('end');
