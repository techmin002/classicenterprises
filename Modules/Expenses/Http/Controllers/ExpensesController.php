...
class ExpensesController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('expenses::expenses.index', compact('expenses', 'categories', 'branches', 'type'));
    }
    public function pettyExpense(Request $request)
    {
        $type = 'petty_exp';
        $user = auth()->user();

        $expenses = Expenses::with('category')
            ->where('mode', 'petty cash');

        // ✅ Branch filter
        if ($user->role->name === 'Super Admin') {
            if (session('branch_id')) {
                $expenses->where('branch_id', session('branch_id'));
            }
        } else {
        ...
        if ($request->filter == '7days') {
            $expenses->where('created_at', '>=', now()->subDays(7));
        }
        if ($request->filter == '15days') {
            $expenses->where('created_at', '>=', now()->subDays(15));
        }
        if ($request->filter == '1month') {
            $expenses->where('created_at', '>=', now()->subMonth());
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->mode === 'petty cash') {
            ...
            PettyCashTransaction::create([
                ...
                'message' => $expense->title .": " . $expense->description,
    ...
    public function update(Request $request, $id)
    {
        ...
        // --- Get logged-in user branch (same logic as store) ---
        $user = auth()->user();
        if ($user->role->name === 'Super Admin') {
            $branchId = session('branch_id');
        } else {
        ...
        // --- Revert old petty cash if old mode was 'petty cash' ---
        if ($oldMode === 'petty cash') {
            ...
            // Delete old petty cash transaction
            PettyCashTransaction::where('reference_id', $expense->id)
                ->where('type', 'expense')
                ->delete();
        }

        // --- Get new expense date's month/year ---
        ...
        // --- If mode is petty cash, deduct new amount ---
        if ($request->mode === 'petty cash') {
            $pettyCash = PettyCashAdd::where('branch_id', $branchId)
            ...
            if (!$pettyCash) {
                return back()->with('error', 'No petty cash found for the selected month and branch!');
            }
            ...
            PettyCashTransaction::create([
                ...
                'total_cash_before' => $before,
                'remaining_cash_after' => $after,
                'message' => 'Expense entry: ' . $request->title,
                'reference_id' => $expense->id,
                'created_by' => auth()->id(),
            ]);
        }
