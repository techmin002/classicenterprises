        ...
        <div class="modal-content shadow-lg modal-advanced" style="border-radius: 24px; border: none;">
            ...
            <form action="" id="devicePurchaseForm" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body modal-body-advanced">
                    ...
                    <div class="container-fluid">
                        <div id="technicalToolsContainer"></div>
...
<script>
    $(document).ready(function() {
        ...
        function calculateTotals() {
            ...
            // Technical tools subtotal
            $('.technicaltool-row').each(function() {
                const qty = parseFloat($(this).find('.technicaltool-quantity').val()) || 0;
                const price = parseFloat($(this).find('.technicaltool-price').val()) || 0;
                const total = qty * price;
                $(this).find('.technicaltool-total').val(total.toFixed(2));
                technicalToolsTotal += total;
            });

            // Update subtotals
            $('#accessories_subtotal').val(accessoriesTotal.toFixed(2));
            $('#machinery_subtotal').val(machineryTotal.toFixed(2));
            $('#technicaltools_subtotal').val(technicalToolsTotal.toFixed(2));

            // grand total update
            const grandTotal = accessoriesTotal + machineryTotal + technicalToolsTotal;
        ...
        // Add technicaltool row
        $('#addTechnicalTool').click(function() {
            technicalToolIndex++;

            const row = `
    <div class="row gy-4 item-row technicaltool-row" id="technicaltool-row-${technicalToolIndex}">
        <div class="col-lg-4">
            <label class="form-label12 fw-semibold">Tool Name</label>
            <select class="form-control border-info shadow-sm technicaltool-name"
                    name="technicaltools[${technicalToolIndex}][id]">
                <option value="">Select Tool</option>
                @foreach ($technicaltools as $tool)
                    <option value="{{ $tool->id }}" data-price="{{ $tool->price }}">
                        {{ $tool->tool_name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-lg-2">
            <label class="form-label12 fw-semibold">Quantity</label>
            <input class="form-control border-info shadow-sm technicaltool-quantity"
        ...
        <div class="col-lg-2">
            <label class="form-label12 fw-semibold">Unit Price</label>
            <input class="form-control border-info shadow-sm technicaltool-price"
                   name="technicaltools[${technicalToolIndex}][price]"
                   type="number" step="0.01">
        </div>

        <div class="col-lg-3">
            <label class="form-label12 fw-semibold">Total</label>
            <input class="form-control border-info shadow-sm technicaltool-total"
                   name="technicaltools[${technicalToolIndex}][total]"
        ...
        <div class="col-lg-1 d-flex align-items-end">
            ...
            // auto price
            $(`#technicaltool-row-${technicalToolIndex} .technicaltool-name`).change(function() {
                const price = $(this).find(':selected').data('price');
                $(this).closest('.technicaltool-row')
                    .find('.technicaltool-price')
                    .val(price)
                    .trigger('input');
            });
        });
