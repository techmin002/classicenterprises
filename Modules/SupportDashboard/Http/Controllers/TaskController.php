...
class TaskController extends Controller
{
    ...
    public function create()
    {
        if (auth()->user()->role['name'] === 'Super Admin') {
            // dd('admin');
            $customers = Customer::where('customer_type', 'indoor')->where('branch_id', session('branch_id'))->with('lead', 'branch', 'products')->get();
        } else {
            // dd('user');
            $branch_id = auth()->user()->branch_id;
            $customers = Customer::where('customer_type', 'indoor')->where('branch_id', $branch_id)->with('lead', 'branch', 'products')->get();
        }
    ...
    // {
    //     // dd('TaskController Store');
    //     // dd($request->all());
    //     RegisterCustomerTicket::create([
