<div class="modal fade" id="editCategory{{ $exp->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content" style="border-radius: 8px;">
            ...
            <form action="{{ route('outsidercustomer-assign.store', $exp->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="container">
                        <input type="hidden" name='customer_id' value="{{ $exp->customer_id }}">
                        <div class="row gy-3">
                            <div class="col-lg-6">
                                <label class="form-label">Customer Name:</label>
                                <input type="text" class="form-control" value="{{ $exp->customer_name ?? 'N/A' }}" disabled>
                            </div>
                            @if ($exp->status == 'assign')
                            <div class="col-lg-6">
                                <label class="form-label">Assign Lead:</label>
                                <input type="text" class="form-control" value="{{ $exp->user->name ?? 'No User Assigned' }}" disabled>
                            </div>
                            @endif
                        ...
                        <div class="col-lg-12 mt-2">
                            ...
                            <select class="form-control" name="user_id" required>
                                ...
                                <option value="{{ $user->id }}" {{ isset($value->user_id) && $value->user_id == $user->id ? 'selected' : '' }}>
