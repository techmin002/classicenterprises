...
class CompanyProfileController extends Controller
{
    ...
    public function WhyUsStore(Request $request)
    {
        ...
        Log::create([
            'perform'   => auth()->user()->name
                . 'Why Choose Us Created: ' . $request['title']
                . ' at ' . now(),
            'user_id'   => auth()->user()->id,
            'branch_id' => session('branch_id') ?? auth()->user()->branch_id,
            'url'       => url()->current(),
    ...
    public function WhyUsUpdate(Request $request, $id)
    {
        ...
        Log::create([
            'perform'   => auth()->user()->name
                . 'Why Choose Us Updated: ' . $whyus['title']
                . ' at ' . now(),
            'user_id'   => auth()->user()->id,
            'branch_id' => session('branch_id') ?? auth()->user()->branch_id,
            'url'       => url()->current(),
    ...
    public function WhyUsDelete($id)
    {
        ...
        Log::create([
            'perform'   => auth()->user()->name
                . 'Why Choose Us Deleted: ' . $whyus['title']
                . ' at ' . now(),
            'user_id'   => auth()->user()->id,
            'branch_id' => session('branch_id') ?? auth()->user()->branch_id,
            'url'       => url()->current(),
    ...
    {
        $logs = Log::with('user', 'branch')->latest()->get();
        return view('setting::company-profile.logs', compact("logs"));
    }
}
