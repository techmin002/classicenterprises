                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="btn-group petty-filter" role="group">
                                <a href="{{ route('pettycash-addcash.index', ['filter' => '7days']) }}" class="btn {{ request('filter') == '7days' ? 'active-btn' : '' }}">7 Days</a>

                                <a href="{{ route('pettycash-addcash.index', ['filter' => '15days']) }}" class="btn {{ request('filter') == '15days' ? 'active-btn' : '' }}">15 Days</a>

                                <a href="{{ route('pettycash-addcash.index', ['filter' => '1month']) }}" class="btn {{ request('filter') == '1month' ? 'active-btn' : '' }}">1 Month</a>

                                <button id="customBtn" class="btn {{ request('start_date') && request('end_date') ? 'active-btn' : '' }}" type="button">Custom</button>
                            </div>
                        ...
                        <!-- Custom Date Filter -->
                        <div id="customDateFilter" style="{{ request('start_date') && request('end_date') ? '' : 'display:none;' }}; margin:10px;">
                            <form method="GET" action="{{ route('pettycash-addcash.index') }}" class="row g-2">
                                <div class="col-auto">
                                    ...
                                    <input type="date" id="start_date" name="start_date" class="form-control shadow-sm rounded" value="{{ request('start_date') }}">
                                </div>
                                <div class="col-auto">
                                    <label for="end_date" class="form-label fw-bold text-dark">End Date:</label>
                                    <input type="date" id="end_date" name="end_date" class="form-control shadow-sm rounded" value="{{ request('end_date') }}">
                                </div>
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            @if ($value->status == 'on')
                                            <a href="{{ route('pettycash-addcash.status', $value->id) }}" class="btn btn-success btn-sm">On</a>
                                            @else
                                            <a href="{{ route('pettycash-addcash.status', $value->id) }}" class="btn btn-danger btn-sm">Off</a>
                                            @endif
                                        </td>
                                        <td>
                                            @can('edit_pettycash')
                                            <a data-toggle="modal" data-target="#editCategory{{ $value->id }}" class="btn btn-primary btn-sm" data-toggle="tooltip" title="Edit">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                            @endcan
                                            @include('pettycash::cash_add.edit')
                                            @can('delete_pettycash')
                                            <button id="delete" class="btn btn-danger btn-sm" data-toggle="tooltip" title="Delete" onclick="event.preventDefault();
                                                                if (confirm('Are you sure? It will delete the data permanently!')) {
                                                                    document.getElementById('destroy{{ $value->id }}').submit();
                                                                }">
                                                <i class="fa fa-trash"></i>
                                                <form id="destroy{{ $value->id }}" class="d-none" action="{{ route('pettycash-addcash.destroy', $value->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </button>
