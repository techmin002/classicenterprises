...
@section('title', ucfirst($saleType) . ' Installation Queue')
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>{{ ucfirst($saleType) }} Installation Queue</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
                            <li class="breadcrumb-item active">{{ ucfirst($saleType) }} Installation Queue</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                        ...
                        <div class="card">
                            <div class="card-header">
                                ...
                                <div class="btn-group petty-filter" role="group">
                                    <a href="{{ route('installation-queue.index', ['sale_type' => $saleType, 'filter' => '7days']) }}"
                                        class="btn {{ request('filter') == '7days' ? 'active-btn' : '' }}">7 Days</a>

                                    <a href="{{ route('installation-queue.index', ['sale_type' => $saleType, 'filter' => '15days']) }}"
                                        class="btn {{ request('filter') == '15days' ? 'active-btn' : '' }}">15 Days</a>

                                    <a href="{{ route('installation-queue.index', ['sale_type' => $saleType, 'filter' => '1month']) }}"
                            ...
                            <div id="customDateFilter"
                                ...
                                <form method="GET"
                                    action="{{ route('installation-queue.index', ['sale_type' => $saleType]) }}"
                                    class="row g-2">
                                    <div class="col-auto">
                                    ...
                                    <tbody>
                                        @foreach ($customers as $key => $exp)
                                            <tr>
                                                ...
                                                <td class="text-center">{{ $exp->lead->name ?? 'N/A' }}</td>
                                                <td class="text-center">{{ $exp->lead->mobile ?? 'N/A' }}</td>
                                                <td class="text-center">{{ $exp->lead->address ?? 'N/A' }}</td>
                                                ...
                                                <td class="text-center">{{ $exp->total_amount ?? '0' }}</td>
                                                <td class="text-center">{{ $exp->paid_amount ?? '0' }}</td>
                                                <td class="text-center text-danger">{{ $exp->due_amount ?? '0' }}</td>
                                                <td>
                                                    ...
                                                    @php
                                                        ...
                                                        $notes = Modules\Lead\Entities\CustomerNote::where(
                                                            'lead_id',
                                                            $exp->lead->id,
                                                        )
                                                            ...
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title"
                                                                        id="noteModalLabel{{ $exp->id }}">Lead Note
                                                                    </h5>
                                                                ...
                                                                <div class="modal-body">
                                                                    ...
                                                                    <!-- Form for new note -->
                                                                    <form
                                                                        action="{{ route('leads.message.update', $exp->id) }}"
                                                                        method="POST">
                                                                        @csrf
                                                                        @method('PUT')
